...
import {
  ...
  FaFileContract, FaPlus, FaEyeSlash, FaHome, FaEnvelope, FaPhone, FaIdBadge
} from "react-icons/fa";
...
export default function AdminTenants() {
  ...
  const [confirmCreate, setConfirmCreate] = useState(false); // New confirmation state
  ...
  // The actual API call executed by the GeneralConfirmationModal
  const executeCreate = async () => {
    ...
    try {
      ...
      setConfirmCreate(false); // Close confirmation so user can see error
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          #tenant-print-area { position: absolute; left: 0; top: 0; width: 100%; font-family: 'Inter', system-ui, -apple-system, sans-serif !important; color: #0f172a; }
      ...
      {/* ── SCREEN UI ── */}
      <div className="w-full h-full bg-[#f8fafc] p-4 md:p-6 text-slate-800 font-sans flex flex-col no-print min-h-screen overflow-x-hidden">
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={7} className="py-24 text-center">
                        <div className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm">
                    ...
                    paginated.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-slate-100 text-xs font-bold text-slate-600 shadow-sm">
                            {r.occupancy}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 text-right">
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button title="Delete Tenant" onClick={() => setDeleteTarget(r)}
                ...
                paginated.map((r) => (
                  <div key={r.id} className="p-5 space-y-4 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="grid grid-cols-2 gap-3 bg-white p-3 rounded-xl border border-slate-100">
                      <div className="min-w-0">
                        <p className="text-[9px] text-slate-400 uppercase tracking-widest font-bold mb-0.5">Contact Info</p>
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={() => navigate(`/admin/tenants/${r.id}`)}
                        className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold active:scale-[0.98] transition-transform">
      ...
      {createModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[40] p-4 no-print animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            {/* Form intercepts submit and opens Confirm Modal */}
            <form onSubmit={handlePreSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-6">
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 justify-end pt-6 border-t border-slate-100 mt-8">
                <button type="button" onClick={() => { setCreateModal(false); setCreateForm(EMPTY_FORM); setCreateError(""); }}
                  className="px-6 py-3 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors w-full sm:w-auto">Cancel</button>
                <button type="submit" disabled={submitting}
                  className="px-8 py-3 rounded-xl bg-[#db6747] text-white text-xs font-bold hover:bg-[#c45a3a] transition-colors shadow-lg active:scale-95 disabled:opacity-60 w-full sm:w-auto uppercase tracking-wider">
                  Create Tenant
                </button>
              </div>
            </form>
      ...
      <GeneralConfirmationModal
        ...
        message={`Are you sure you want to approve and generate an account for ${createForm.fullName || "this tenant"}?`}
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`text-[9px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider border shrink-0 ${getStyle(status)}`}>
      {status || "No Contract"}
    </span>
  );
}